...
from vectorstore.pinecone_store import get_pinecone_retriever, is_pinecone_enabled
    ...
    def built_retriver( self,
        ...
        *,
        ...
        try:
            ...
            # ── PII redaction: strip names / emails / phones before LLM ──
            redactor = get_redactor()
            docs, pii_count = redactor.redact_documents(docs)
            if pii_count:
                log.info("PII entities redacted from documents", count=pii_count,
                         session_id=self.session_id)

            chunks = self._split(docs, chunk_size=chunk_size, chunk_overlap=chunk_overlap)

            # ── Vector store: Pinecone (cloud) or FAISS (local) ──────────────
            if is_pinecone_enabled():
                retriever = get_pinecone_retriever(self.session_id, chunks, k=k)
                log.info("Pinecone retriever built", session_id=self.session_id)
                return retriever
